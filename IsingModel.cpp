...
{ 
  ...
  srand(time(NULL));
  int lattice_array[N][N]; 
  for(kT = .01;kT<max_kT;kT+=.1)
    {
      double beta = 1/(kT);
      printf("Temp: %f\n",kT);
      ...
      for(int runs =0; runs<50;runs++)
	{
	  for (int count_1=0; count_1<N;count_1++)
		...
		{lattice_array[count_1][count_2] = 1;}	   
	    }
	  ...
	  for(int sample=0;sample<max_samples;sample++)
	  {
	    ...
	    if (delta_E<0)
	      {lattice_array[indx_1][indx_2] = -1*lattice_array[indx_1][indx_2];}
	    else
	      {
	  ...
	  for(int count_1 = 0; count_1<N; count_1++)
	    {
	      for(int count_2 = 0; count_2<N;count_2++)
		{spin_sum += lattice_array[count_1][count_2];}
	    }
